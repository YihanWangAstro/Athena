...
namespace {
    // Declarations
    bool ConservedToPrimitiveNormal(const AthenaArray<Real> &dd_vals, const AthenaArray<Real> &ee_vals,
                                    const AthenaArray<Real> &mm_vals, const AthenaArray<Real> &bb_vals,
                                    const AthenaArray<Real> &tt_vals, Real gamma_adi, Real pgas_old, int k, int j,
                                    int i, AthenaArray<Real> &prim, Real *p_gamma_lor, Real *p_pmag);
}  // namespace
    ...
    for (int k = kl; k <= ku; ++k) {
        for (int j = jl; j <= ju; ++j) {
            ...
            for (int i = il; i <= iu; ++i) {
                ...
                normal_mm_(0, i) = SQR(cons(IM1, k, j, i)) + SQR(cons(IM2, k, j, i)) + SQR(cons(IM3, k, j, i));
                normal_mm_(1, i) = cons(IM1, k, j, i);
                normal_mm_(2, i) = cons(IM2, k, j, i);
                normal_mm_(3, i) = cons(IM3, k, j, i);
                normal_bb_(0, i) = SQR(bb_cc(IB1, k, j, i)) + SQR(bb_cc(IB2, k, j, i)) + SQR(bb_cc(IB3, k, j, i));
                normal_bb_(1, i) = bb_cc(IB1, k, j, i);
                normal_bb_(2, i) = bb_cc(IB2, k, j, i);
                normal_bb_(3, i) = bb_cc(IB3, k, j, i);
                normal_tt_(i) = cons(IM1, k, j, i) * bb_cc(IB1, k, j, i) + cons(IM2, k, j, i) * bb_cc(IB2, k, j, i) +
            ...
            for (int i = il; i <= iu; ++i) {
                ...
                Real ee_min = density_floor_local + pressure_floor_local / (gamma_adi - 1.0) + 0.5 * normal_bb_(0, i);
                ...
                // Set primitives
                Real gamma, pmag;
                bool success = ConservedToPrimitiveNormal(normal_dd_, normal_ee_, normal_mm_, normal_bb_, normal_tt_,
                ...
                Real rho_add = std::max(density_floor_local - prim(IDN, k, j, i), static_cast<Real>(0.0));
                Real pgas_add = std::max(pressure_floor_local - prim(IPR, k, j, i), static_cast<Real>(0.0));
                if (success && (rho_add > 0.0 || pgas_add > 0.0)) {
                    ...
                    success = ConservedToPrimitiveNormal(normal_dd_, normal_ee_, normal_mm_, normal_bb_, normal_tt_,
                                                         gamma_adi, pgas_old, k, j, i, prim, &gamma, &pmag);
...
void EquationOfState::PrimitiveToConserved(const AthenaArray<Real> &prim, const AthenaArray<Real> &bc,
                                           AthenaArray<Real> &cons, Coordinates *pco, int il, int iu, int jl, int ju,
                                           int kl, int ku) {
...
void EquationOfState::FastMagnetosonicSpeedsGR(Real wgas, Real pgas, Real u0, Real u1, Real b_sq, Real g00, Real g01,
                                               Real g11, Real *p_lambda_plus, Real *p_lambda_minus) {
...
namespace {
    ...
    bool ConservedToPrimitiveNormal(const AthenaArray<Real> &dd_vals, const AthenaArray<Real> &ee_vals,
        ...
        if (!std::isfinite(prim(IVX, k, j, i)) || !std::isfinite(prim(IVY, k, j, i)) ||
